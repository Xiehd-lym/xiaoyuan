	...
	<head>
		...
		<script language="javascript">
           function YuangongDel(id)
           {
               if(confirm('您确定删除吗？'))
               {
                   window.location.href="<%=path %>/yuangong/del.action?id="+id;
               }
           }
           
           function YuangongEdit(id)
           {
							window.location.href="<%=path %>/yuangong/edit.action?id="+id;
           }
           
           function YuangongAdd()
           {
                 var url="<%=path %>/yuangong/add.action";
	...
	<body>
		<div class="col-lg-12">
			...
			<div class="row search"  style="margin-bottom:30px;">
					<div class="col-md-10">
						<form method="post" class="form-inline" action="<%=path%>/yuangong/list.action" method="post">
		...
		<div>
				<table class="table table-hover  table-bordered table-striped" style="margin-bottom: 0px;">
					...
					<c:forEach items="${pagers.datas}" var="Yuangong">
					<tr>
						...
						<td>
							<button type="button" class="btn btn-info btn-xs" onclick="YuangongEdit(${Yuangong.id})"><span class="icon-edit text-blue"></span>修改</button>
							<button type="button" class="btn btn-info btn-xs" onclick="YuangongDel(${Yuangong.id})"> <span class="icon-trash-o text-red"></span>删除</button>
						</td>
					</tr>
					</c:forEach>
				</table>
				...
				<nav>
					<ul class="pagination">
				      <pg:pager  url="${ctx}/yuangong/list.action" maxIndexPages="5" items="${pagers.total}"  maxPageItems="15" export="curPage=pageNumber" >
